SUBROUTINE getgrid(myid,Nx,Ny,Nz,Lx,Ly,Lz,pi,name,x,y,z,kx,ky,kz,decomp)
	...
	! pseudospectral simulation of the Brusselator equations
	!
	! u_t = a + u^2v - (b+1)u + Du (u_{xx} + u_{yy} + u_{zz})
    ! v_t = bu - u^2v + Dv (v_{xx} + v_{yy} + v_{zz})
	!
	! The boundary conditions are 
	!   u(x=-Lx*pi,y,z)=u(x=Lx*\pi,y,z), 
	!	u(x,y=-Ly*pi,z)=u(x,y=Ly*pi,z),
	!   u(x,y,z=-Ly*pi)=u(x,y,z=Ly*pi),
	!   v(x=-Lx*pi,y,z)=v(x=Lx*\pi,y,z), 
	!	v(x,y=-Ly*pi,z)=v(x,y=Ly*pi,z),
	!   v(x,y,z=-Ly*pi)=v(x,y,z=Ly*pi),
	!
	! INPUT
	!
	! .. Scalars ..
	...
	IF (myid.eq.0) THEN
	 ...
	 ind=index(name,' ') -1
     name_config=name(1:ind)//'xcoord.dat' 
	 ...
	 ind=index(name,' ') -1
     name_config=name(1:ind)//'ycoord.dat' 
	 OPEN(unit=11,FILE=name_config,status="UNKNOWN") 	
	 REWIND(11)
	 DO j=1,Ny
	 ...
	 ind=index(name,' ') -1
     name_config=name(1:ind)// 'zcoord.dat' 
	 OPEN(unit=11,FILE=name_config,status="UNKNOWN") 	
	 REWIND(11)
	 DO k=1,Nz
